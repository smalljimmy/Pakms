...
namespace ICP.Controls.PrintableListView
{
    public class UIUtility
    {
        ...
        public static ListView createListView(IList sourceView, ViewBase view, int itemsApproximation, ref int itemIndex, Rect size, out BindingList<object> list, Dictionary<string, DataTemplate> cellTemplates = null)
        {
            ...
            if (view != null)
            {
                ...
                // Apply print templates js/goco
                if (cellTemplates != null)
                {
                    foreach (var column in ((GridView)listview.View).Columns)
                    {
                        if (cellTemplates.ContainsKey(column.Header.ToString()))
                            column.CellTemplate = cellTemplates[column.Header.ToString()];
                    }
                }
