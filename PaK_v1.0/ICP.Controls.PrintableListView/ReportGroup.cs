...
namespace ICP.Controls.PrintableListView
{
    public class ReportGroup : DependencyObject
    {
        ...
        internal FrameworkElement Render(PrintableListView printableListView, object parentDataContext, CollectionView sourceView, ViewBase view, int itemsApproximation, ref int itemIndex, Rect size, out BindingList<object> list)
        {
            ...
            Dictionary<string, DataTemplate> cellTemplates = new Dictionary<string, DataTemplate>()
            {
                {"ID", printableListView.myCellTemplateID},
                {"Price", printableListView.myCellTemplatePRICE},
            };

            ListView listview = UIUtility.createListView(group.Items, UIUtility.CreateDeepCopy<ViewBase>(view), itemsApproximation, ref itemIndex, new Rect(size.Left, size.Top, size.Width, pageHeight), out list, cellTemplates);
